...
function fetchTracecLogsForCinder(index,callback){
...
function fetchCinderLogs(req,res){
    ...
    Sync(function() {
        ...
        var index='cinderindex-'+MyDateString;

        var resultForErrorsTable=fetchTracecLogsForCinder.sync(null,index);
...
exports.fetchCinderLogs=fetchCinderLogs;


function cinder(req,res){
    if(req.session.user) {
        res.render("Cinder.ejs",{username:"abc"});
    }
    else{
        res.redirect("/login");
    }
